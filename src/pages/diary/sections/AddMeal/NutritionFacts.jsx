...
import { changeFirstLetterToLower } from "../../../../utils/helpers";

/* eslint-disable react/prop-types */
export function MealNutrFacts() {
  const [factID, setFactID] = useState(null);
  const { mealObject } = useContext(DiaryContext);
      ...
      >
        ...
        >
          <Fact
            ...
            icon="energy.png"
            name="Energy"
            value={`${mealObject?.energy ? mealObject?.energy : "0"}kcal`}
          ...
          <Fact
            ...
            icon="carbs.png"
            name="Carbs"
            value={`${mealObject?.carbs ? mealObject?.carbs : "0"}g`}
            handleEdit={handleClick}
          />
          <Fact
            id={3}
            icon="protein.png"
            name="Protein"
            value={`${mealObject?.protein ? mealObject?.protein : "0"}g`}
            handleEdit={handleClick}
          />
          <Fact
            id={4}
            icon="fats.png"
            name="Fats"
            value={`${mealObject?.fats ? mealObject?.fats : "0"}g`}
            handleEdit={handleClick}
          />
        </div>
      </div>
...
function EditFact({ title, onCancel }) {
  ...
  const { setMealObject } = useContext(DiaryContext);

  function handleSetFact(title) {
    setMealObject((prevObj) => ({
      ...prevObj,
      [changeFirstLetterToLower(title)]: factValue,
    }));
  }
    ...
    border-b border-grey-6 outline-none"
          ...
          <p className="text-xs font-medium text-grey-4">
            ...
            {title === "Energy"
              ...
              ? "protein "
              : "fat "}{" "}
            content of your meal in {title === "Energy" ? "kcal" : "gram"}{" "}
          </p>
        </div>
      </Modal>
    </ScreenOverlay>
